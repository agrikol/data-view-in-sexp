...
Синтаксис: (Name (:attribute 22) (child "Ivan"))
2. Листовой узел (leaf):
Узел либо:
...
- Дочерний узел: Вложенный список, начинающийся с имени (строка без :).
...
- Узел не может содержать одновременно листовое значение и (атрибуты или дети).
