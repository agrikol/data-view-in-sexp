...
class Validator:
    ...
    def validate(self) -> bool:
        self._check_name(self.document.name, self.schema.name)
        self._check_value(self.document, self.schema)
        self._check_attrs(self.document, self.schema)
        self._check_children(self.document, self.schema)
        return True
    ...
    def _check_value(self, document: Node, schema: SchemaNode) -> None:
        ...
        py_type = TYPE_MAP.get(schema.value_type)
    ...
    def _check_attrs(self, document: Node, schema: SchemaNode) -> None:
        ...
        for name, value in document.attrs.items():
            ...
            attr_schema = schema.attrs[name]
            py_type = TYPE_MAP.get(attr_schema.value_type)
    ...
    def _check_children(self, document: Node, schema: SchemaNode) -> None:
        ...
        for child_schema in schema.children:
            ...
            for child in doc_children.get(child_schema.name, []):
                self.validate(child, child_schema)
